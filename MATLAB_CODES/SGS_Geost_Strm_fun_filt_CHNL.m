%%%%%%%% THIS SCRIPT GENERATES SUBGRID SCALE EDDY FLUXES IN CHANNEL
...
tsize = 730; %%730 Maximum number of years for <= 1/4 deg tsize=3650 and for 1/16 tsize=730, tsize=50 for 1/8 deg. 730 for 1/16 %%% 1825 for 1/8
nyr = 10;   %% Do not change this at all! 
avyr = 0.25;  %% !!!!!! for 1/8 in movmean use 0.25 and for 1/16 and 1/4 (3650) use 0.25/4 
TS = 365;%%365;%floor( tsize * ((nyr-avyr)/nyr) ); %% time average for TS<t<tsize
...
sea_surf_height = ncread(openstr,'e',[1,1,1,396],[xsize,ysize,1,1]); %% 396
bathymetry      = ncread(openstr,'e',[1,1,7,396],[xsize,ysize,1,1]); %% 396 
...
for i= 1:tsize
        if (i>=TS)
            ...
            %MASK_eps(e-repmat(ee(:,:,7),[1,1,6])<eps) = NaN;
            ...
            [ugh_sgs,vgh_sgs,h_fxy,vg_fxy,vg,ugh_fxy,ug_fxyh_fxy,vgh_fxy,vg_fxyh_fxy,ug_sgs,vg_sgs,h_sgs]=SGSflux_cheap(eee,CORCOEF,gprime,DX,DY,nfxy,eps,1); %%% crops 1 is the best option 
            ...
            [ugug_sgs,vgvg_sgs]=SGSeddvis_cheap(eee,CORCOEF,gprime,DX,DY,nfxy,eps,1);
            ...
            [dhdxreg,dhdyreg]=grad(h_fxyreg,DX_fxy,DY_fxy);
...
[ugh_sgsta_ST,vgh_sgsta_ST,hta_fxy_ST,vgta_fxy_ST,vgta_ST,ugh_fxyta_ST,ug_fxyh_fxyta_ST,vgh_fxyta_ST,vg_fxyh_fxyta_ST,ug_sgsta_ST,vg_sgsta_ST,h_sgsta_ST]=SGSflux_cheap(eeta,CORCOEF,gprime,DX,DY,nfxy,eps,1); %%%%% crops 1 is the best so far

[ugug_sgsta_ST,vgvg_sgsta_ST]=SGSeddvis_cheap(eeta,CORCOEF,gprime,DX,DY,nfxy,eps,1);
...
for i= 1:tsize
        if (i>=TS)
            ...
            [ugh_sgs_TR,vgh_sgs_TR,h_fxy_TR,vg_fxy_TR,vg_TR,ugh_fxy_TR,ug_fxyh_fxy_TR,vgh_fxy_TR,vg_fxyh_fxy_TR,ug_sgs_TR,vg_sgs_TR,h_sgs_TR]=SGSflux_cheap(eee2,CORCOEF,gprime,DX,DY,nfxy,eps,1); %%% crops 1 is the best option 
            [ugug_sgs_TR,vgvg_sgs_TR]=SGSeddvis_cheap(eee2,CORCOEF,gprime,DX,DY,nfxy,eps,1); 
...
nom_sgsta_G = nom_sgsta_G .* MASK_GA .* MASK; % .* (h_fxyta / mean_h_fxyta);  %%%%% WEIGHTING BY THICKNESS HEIGHT
...
%%%%%%%%%%%%%%%%%% unfiltered and filtered vg
vg_surf = vg(:,:,1);
vg_fxy_surf = vg_fxy(:,:,1);
vg_sgs_surf = vg_sgs(:,:,1);
vg_surf(vg_surf==0) = NaN;
vg_fxy_surf(vg_fxy_surf==0) = NaN;
vg_sgs_surf(vg_sgs_surf==0) = NaN;
figure(22456)
contourf(X,Y',vg_surf',[-1.7:0.05:1.7],'edgecolor','none');
...
hold on
figure(22457)
contourf(X,Y',vg_fxy_surf',[-1.7:0.05:1.7],'edgecolor','none');
set(gca,'clim',[-0.3,0.3]);
colormap(jet(15));
colorbar;
%set(gca,'clim',[-4.5,4.5]);
hold on
figure(22458)
contourf(X,Y',vg_sgs_surf',[-1.7:0.05:1.7],'edgecolor','none');
set(gca,'clim',[-0.3,0.3]);
colormap(jet(15));
colorbar;
%set(gca,'clim',[-4.5,4.5]);
